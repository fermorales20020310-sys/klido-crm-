...
// Si WABA_ID empieza con EAAT es que pusiste el token mal - lo ignoramos y lo auto-descubrimos
if(WABA_ID.startsWith('EAAT')||WABA_ID.length>50){ console.log('⚠️ WABA_ID parece ser un TOKEN, lo ignoramos y lo auto-descubrimos...'); WABA_ID=''; }

console.log('=== KLIDO 100% META AUTO WABA ===');
console.log('PHONE_ID:',PHONE_ID,'WABA manual:',WABA_ID||'auto','TOKEN:',!!META_TOKEN);
...
try{
 if(process.env.DATABASE_URL){
  ...
  (async()=>{
   ...
   await pool.query(`CREATE TABLE IF NOT EXISTS campaigns(id TEXT PRIMARY KEY, name TEXT, total INT, sent INT, delivered INT, read INT, failed INT, status TEXT, created_at TIMESTAMP DEFAULT NOW())`);
   ...
   console.log('✅ HISTORIAL DB ACTIVO:', rows.length, 'mensajes');
  })();
 }
}catch{}
...
async function discoverWABA(){
 ...
 try{
  // 100% API META - descubre el WABA ID desde el PHONE_ID
  const r=await fetch(`https://graph.facebook.com/v20.0/${PHONE_ID}?fields=whatsapp_business_account{id}`,{
   headers:{Authorization:`Bearer ${META_TOKEN}`}
  });
  const j=await r.json();
  console.log('Auto WABA discovery:', JSON.stringify(j));
  if(j.whatsapp_business_account?.id){
   WABA_ID=j.whatsapp_business_account.id;
   console.log(`✅ WABA_ID auto-descubierto por API META: ${WABA_ID}`);
   return WABA_ID;
  }
 }catch(e){ console.log('discover WABA err',e.message); }
 return null;
}
...
async function fetchMetaTemplates(){
 try{
  ...
  if(!waba||!META_TOKEN){ console.log('Sin WABA aún'); return cacheTemplates; }
  const r=await fetch(`https://graph.facebook.com/v20.0/${waba}/message_templates?fields=name,status,language,category,components&limit=100`,{
   headers:{Authorization:`Bearer ${META_TOKEN}`}
  });
  const j=await r.json();
  if(j.error){ console.log('Error templates API:', j.error.message, j.error); return cacheTemplates; }
  if(j.data){
   ...
   console.log(`✅ ${cacheTemplates.length} plantillas APROBADAS del API:`, cacheTemplates.map(t=>t.name).join(', '));
  }
  return cacheTemplates;
 }catch(e){ console.log('templates fetch err',e.message); return cacheTemplates; }
}
...
app.post('/webhook',async(req,res)=>{
 ...
 if(value.messages){ const m=value.messages[0]; const contact=value.contacts?.[0]; const wa_id=m.from; const name=contact?.profile?.name||wa_id; let txt=m.type==='text'?m.text.body:`[${m.type}]`; saveHistory({wa_id,text:txt,type:m.type,direction:'in',source:'inbox',created_at:new Date(),status:'delivered'}); console.log(`💬 HISTORIAL: ${wa_id} ${txt}`); }
 res.sendStatus(200);
});

app.get('/api/templates',async(_,res)=>{ const d=await fetchMetaTemplates(); res.json(d.length?d:[{name:'alion_co',status:'APPROVED',language:'es',hasImage:true,bodyText:'Hola {{1}} - campaña oficial'}]); });
...
app.get('/api/chats',(_,res)=>{ const map={}; memMessages.forEach(m=>{ if(!map[m.wa_id]||new Date(m.created_at)>new Date(map[m.wa_id].created_at)) map[m.wa_id]={wa_id:m.wa_id,text:m.text,created_at:m.created_at,source:m.source,status:m.status,name:memContacts[m.wa_id]?.name||m.wa_id}; }); res.json(Object.values(map).sort((a,b)=>new Date(b.created_at)-new Date(a.created_at))); });
...
app.post('/api/send',async(req,res)=>{ const {to,message}=req.body; try{ const r=await fetch(`https://graph.facebook.com/v20.0/${PHONE_ID}/messages`,{method:'POST',headers:{Authorization:`Bearer ${META_TOKEN}`,'Content-Type':'application/json'},body:JSON.stringify({messaging_product:'whatsapp',to,type:'text',text:{body:message}})}); const j=await r.json(); saveHistory({wa_id:to,text:message,type:'text',direction:'out',source:'inbox',created_at:new Date(),status:j.messages?'sent':'failed'}); res.json({ok:!!j.messages,data:j}); }catch(e){ res.json({ok:false}); } });
...
app.post('/api/campaigns/send-bulk',async(req,res)=>{
 const {numbers,templateName}=req.body; const tpl=templateName||'alion_co'; const id=Date.now().toString();
 ...
 (async()=>{ for(const raw of numbers){ const to=String(raw).replace(/\D/g,''); try{ const r=await fetch(`https://graph.facebook.com/v20.0/${PHONE_ID}/messages`,{method:'POST',headers:{Authorization:`Bearer ${META_TOKEN}`,'Content-Type':'application/json'},body:JSON.stringify({messaging_product:'whatsapp',to,type:'template',template:{name:tpl,language:{code:'es'}}})}); const j=await r.json(); if(j.messages){ camp.sent++; saveHistory({wa_id:to,text:`[Plantilla ${tpl}]`,type:'template',direction:'out',source:'campaign',template_name:tpl,status:'sent',created_at:new Date()}); } else camp.failed++; }catch{ camp.failed++; } await new Promise(r=>setTimeout(r,400)); } camp.status='completada'; })();
...
app.get('/health',async(_,res)=>{
 ...
 res.json({ok:true,api:'100% META AUTO WABA + HISTORIAL',verify:VERIFY_TOKEN,phone:PHONE_ID,waba:waba||'auto-descubriendo',hasToken:!!META_TOKEN,templates:cacheTemplates.map(t=>t.name),messages:memMessages.length,hasDB:!!pool,files:fs.readdirSync(publicPath)});
});
app.get('/',(req,res)=>res.sendFile(path.join(publicPath,'login.html')));
app.listen(process.env.PORT||3000,()=>console.log(`🚀 KLIDO AUTO WABA LISTO - buscando WABA con API META...`));
