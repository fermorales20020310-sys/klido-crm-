...
app.get('/api/contacts',(req,res)=>{
  res.json(Object.values(db.contacts).sort((a,b)=>(b.lastTimestamp||0)-(a.lastTimestamp||0)));
});
app.get('/api/messages/:id',(req,res)=>{
  res.json(db.messages[req.params.id]||[]);
  if(db.contacts[req.params.id]){ db.contacts[req.params.id].unread=0; save(); }
});
app.post('/api/send', async (req,res)=>{
  ...
  if(db.contacts[wa_id]){
    ...
    db.contacts[wa_id].hot=false; // AL RESPONDER QUITA ROJO Y PASA A BANDEJA NORMAL
...
app.post('/webhook',(req,res)=>{
  ...
  if(b.object==='whatsapp_business_account'){
    b.entry?.forEach(en=>en.changes?.forEach(ch=>{
      ch.value?.messages?.forEach(m=>{
        const wa_id=m.from;
        let text=m.type==='text'?m.text.body:m.type==='image'?'[image] '+(m.image?.caption||''): `[${m.type}]`;
        const name=ch.value.contacts?.[0]?.profile?.name||wa_id;
        if(!db.contacts[wa_id]){
          db.contacts[wa_id]={wa_id,name,lastMessage:text,lastTimestamp:Date.now(),hot:true,unread:1,fromCampaign:false,campaignName:'',tag:''};
        }else{
          db.contacts[wa_id].lastMessage=text;
          db.contacts[wa_id].lastTimestamp=Date.now();
          db.contacts[wa_id].hot=true; // NUEVO MENSAJE ENTRA A HOY CON ROJO
