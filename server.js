...
<style>body{margin:0;background:#0B57D0;height:100vh;display:flex;align-items:center;justify-content:center;font-family:Arial}.card{background:#fff;padding:26px;border-radius:16px;width:92%;max-width:350px;text-align:center}input{width:100%;padding:11px;margin-top:10px;border:1px solid #ccc;border-radius:8px}button{width:100%;padding:11px;margin-top:12px;background:#0B57D0;color:#fff;border:none;border-radius:8px;font-weight:bold}</style></head><body>
<div class="card"><h2 style="color:#0B57D0">KLIDO AVANZA</h2><input id="u" placeholder="Usuario"><input id="p" type="password" placeholder="Contrasena"><button onclick="login()">ENTRAR</button><p id="er" style="color:red"></p></div>
<script>
async function login(){
 let r=await fetch('/api/login',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({user:document.getElementById('u').value,pass:document.getElementById('p').value})});
 let j=await r.json();
 if(j.ok){localStorage.setItem('klido','1'); location.href='/';} else {document.getElementById('er').innerText='Usuario o clave mal';}
}
</script></body></html>
`);
});
...
res.send(`
<!DOCTYPE html><html><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1">
<style>body{margin:0;font-family:Arial;background:#f0f2f5}.top{background:#0B57D0;color:#fff;padding:12px;display:flex;align-items:center}.box{max-width:680px;margin:15px auto;background:#fff;padding:18px;border-radius:12px}textarea{width:100%;padding:10px;border-radius:8px;border:1px solid #ccc}table{width:100%;border-collapse:collapse;font-size:13px;margin-top:10px}th,td{padding:8px;border-bottom:1px solid #eee}</style></head><body>
...
<label>Contactos (Nombre, Numero - uno por linea)</label>
...
async function load(){
 ...
 for(let i=data.length-1;i>=0;i--){
  let c=data[i];
  h+='<tr><td>'+c.fecha+'</td><td>'+c.sent+'</td><td>'+c.sent+'</td><td>'+c.estado+'</td></tr>';
 }
...
async function cargar(){
 ...
 let h='';
 if(chats.length==0){document.getElementById('lista').innerHTML='<p style=padding:10px>No hay chats</p>';return;}
 for(let i=0;i<chats.length;i++){
  let c=chats[i];
  h+='<div class=chat onclick="abrir(\\''+c.tel+'\\')"><b>'+c.nombre+'</b><br>'+c.tel+'</div>';
 }
...
function abrir(tel){
 actual=tel;
 let c=null;
 for(let i=0;i<chats.length;i++){if(chats[i].tel==tel) c=chats[i];}
 let h='';
 for(let j=0;j<c.msgs.length;j++){
  let m=c.msgs[j];
  let cl=m.from=='yo'?'yo':'cli';
  h+='<div class="msg '+cl+'">'+m.text+'</div>';
 }
...
app.post('/api/login',(req,res)=>{
 if(req.body.user===USER && req.body.pass===PASS) res.json({ok:true});
...
app.post('/webhook',(req,res)=>{
 try{
  let v=req.body.entry?.[0]?.changes?.[0]?.value;
  let m=v?.messages?.[0];
  if(m){
   let tel=m.from;
   let txt=m.text?.body||'archivo';
   let nm=v.contacts?.[0]?.profile?.name||tel;
   if(!chats[tel]) chats[tel]={tel:tel,nombre:nm,msgs:[]};
...
app.post('/api/send', async (req,res)=>{
 try{
  await fetch('https://graph.facebook.com/v20.0/'+PHONE_ID+'/messages',{
   ...
   body:JSON.stringify({messaging_product:'whatsapp',to:req.body.to,type:'text',text:{body:req.body.text}})
  });
  if(!chats[req.body.to]) chats[req.body.to]={tel:req.body.to,nombre:req.body.to,msgs:[]};
  chats[req.body.to].msgs.push({from:'yo',text:req.body.text});
  res.json({ok:true});
 ...
 for(let c of req.body.contacts){
  try{
   await fetch('https://graph.facebook.com/v20.0/'+PHONE_ID+'/messages',{
    method:'POST',
    headers:{'Content-Type':'application/json','Authorization':'Bearer '+TOKEN},
    body:JSON.stringify({messaging_product:'whatsapp',to:c.tel,type:'template',template:{name:'alion_co',language:{code:'es_CO'}}})
   });
   sent++;
