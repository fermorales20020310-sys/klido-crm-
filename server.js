...
function getAgency(pid){
  try{
    const m=JSON.parse(process.env.AGENCY_MAP||'{}');
    if(pid&&m[pid]) return m[pid];
    return process.env.DEFAULT_AGENCY||'acol';
...
async function init(){
  ...
  try{
    ...
    if(adminUser && hash){
      await pool.query(`INSERT INTO users(agency_id,username,password_hash,role) VALUES($1,$2,$3,'jefe') ON CONFLICT(agency_id,username) DO UPDATE SET password_hash=$3, role='jefe'`,[adminAgency,adminUser,hash]);
      console.log('admin OK:',adminUser);
    }
  }catch(e){ console.error('admin err',e.message); }
  console.log('GOLD ON');
}
init();
...
app.post('/webhook',async(req,res)=>{
  try{
    ...
    const ag=getAgency(v?.metadata?.phone_number_id);
    if(m){
...
app.get('/api/chats',async(req,res)=>{
  const ag=req.query.agency_id;
  const r=await pool.query(`SELECT wa_id,name,last_message as "lastMessage",unread,unread_dot as dot,last_type,tag FROM conversations WHERE agency_id=$1 ORDER BY last_time DESC LIMIT 300`,[ag]);
  res.json(r.rows);
});

app.get('/api/messages/:wa',async(req,res)=>{
  const r=await pool.query(`SELECT text,direction,timestamp,media_type,media_url,status,is_campaign FROM messages WHERE wa_id=$1 AND agency_id=$2 ORDER BY timestamp ASC LIMIT 1000`,[req.params.wa,req.query.agency_id]);
  res.json(r.rows);
});

app.post('/api/messages/send',async(req,res)=>{
  const {wa_id,text,agency_id}=req.body;
  let pid=process.env.PHONE_NUMBER_ID;
...
app.get('/api/templates',async(req,res)=>{
  ...
  try{
    ...
    const r=await axios.get(`https://graph.facebook.com/${G}/${waba}/message_templates?fields=name,status,language`,{headers:{Authorization:`Bearer ${process.env.WHATSAPP_TOKEN}`}});
...
app.post('/api/campaigns/upload',async(req,res)=>{
  try{
    ...
    for(let p of phones){
      await pool.query(`INSERT INTO campaign_queue(campaign_id,agency_id,wa_id,template,status,created_at) VALUES($1,$2,$3,$4,'queued',$5)`,[cr.rows[0].id,ag,p,template,now]);
    }
    res.json({ok:true,total:phones.length});
  }catch(e){
    console.error(e);
    res.status(500).json({error:'excel_error'});
  }
});

app.get('/api/campaigns',async(req,res)=>{
  const r=await pool.query(`SELECT * FROM campaigns WHERE agency_id=$1 ORDER BY created_at DESC LIMIT 50`,[req.query.agency_id]);
  res.json(r.rows);
});
...
cron.schedule('0 */6 * * *',async()=>{
  ...
  for(let row of q.rows){
    try{
      ...
      await pool.query(`UPDATE campaign_queue SET status='error' WHERE id=$1`,[row.id]);
    }
  }
});

const PORT=process.env.PORT||3000;
app.listen(PORT,()=>console.log('GOLD ON '+PORT));
