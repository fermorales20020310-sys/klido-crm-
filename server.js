...
// ===== LOG PARA VER SI EXISTE CAMPAIGNS.HTML =====
const publicPath = path.join(__dirname, 'public');
if (fs.existsSync(publicPath)) {
  console.log('✅ Archivos en public:', fs.readdirSync(publicPath));
} else {
  console.log('❌ NO EXISTE CARPETA public');
}
...
} catch (e) { console.log('No DB', e.message); }
...
// LOGIN - admin@klido.co / klido123
app.post('/api/login', (req, res) => {
  const { email, password } = req.body;
  const validEmail = process.env.ADMIN_EMAIL || 'admin@klido.co';
  const validPass = process.env.ADMIN_PASS || 'klido123';
  console.log('Login attempt:', email);
  if (email === validEmail && password === validPass) return res.json({ ok: true });
...
app.post('/webhook', async (req, res) => {
  try {
    ...
    if (pool) {
      try {
        await pool.query(`INSERT INTO contacts(wa_id,name) VALUES($1,$2) ON CONFLICT(wa_id) DO NOTHING`, [m.from, m.from]);
        await pool.query(`INSERT INTO messages(wa_id,text,file_url,type,direction,source) VALUES($1,$2,$3,$4,'in','inbox')`, [m.from, txt, url, m.type]);
      } catch { memMessages.push({ wa_id: m.from, text: txt, file_url: url, type: m.type, direction: 'in', source: 'inbox', created_at: new Date() }); }
    } else memMessages.push({ wa_id: m.from, text: txt, file_url: url, type: m.type, direction: 'in', source: 'inbox', created_at: new Date() });
...
app.post('/api/campaigns/upload', async (req, res) => {
  ...
  else memCampaigns.unshift({ id: Date.now(), name: name || `Campaña`, total: t, sent: t, created_at: new Date().toISOString() });
  res.json({ ok: true });
});

app.post('/api/campaigns/send-bulk', async (req, res) => {
  const { numbers, message, template } = req.body;
  let sent = 0;
  console.log(`Enviando campaña ${template} a ${numbers?.length} números`);
...
// ===== RUTAS FRONTEND - ESTO ARREGLA EL NOT FOUND =====
app.get('/campaigns', (req, res) => {
  console.log('GET /campaigns -> campaigns.html');
  res.sendFile(path.join(publicPath, 'campaigns.html'));
});

app.get('/campaigns.html', (req, res) => {
  console.log('GET /campaigns.html -> campaigns.html');
  res.sendFile(path.join(publicPath, 'campaigns.html'));
});
...
app.get('/health', (req, res) => {
  let files = [];
  try { files = fs.readdirSync(publicPath); } catch {}
  res.json({ ok: true, files, hasCampaigns: files.includes('campaigns.html') });
});

app.get('/', (req, res) => res.sendFile(path.join(publicPath, 'login.html')));

const PORT = process.env.PORT || 3000;
app.listen(PORT, () => console.log(`KLIDO CRM PRO OK en ${PORT} - CAMPAÑA FIXED`));
