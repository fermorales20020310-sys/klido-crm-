...
<style>body{margin:0;font-family:Arial;background:#f0f2f5}.top{background:#0B57D0;color:#fff;padding:12px;display:flex;align-items:center}.box{max-width:700px;margin:15px auto;background:#fff;padding:18px;border-radius:12px}textarea{width:100%;padding:10px;border-radius:8px;border:1px solid #ccc}table{width:100%;border-collapse:collapse;font-size:13px;margin-top:10px}th,td{padding:8px;border-bottom:1px solid #eee}.upload{border:2px dashed #0B57D0;padding:15px;border-radius:10px;text-align:center;background:#f8fbff;margin-bottom:12px}</style></head><body>
<script>if(!localStorage.getItem('klido')) location.href='/login';</script>
<div class="top"><b>CAMPANAS ALION</b><button onclick="location.href='/'" style="margin-left:auto;background:#fff;color:#0B57D0;border:none;border-radius:12px;padding:6px 12px">Volver</button></div>
<div class="box">
<p style="background:#e8f0fe;padding:8px;border-radius:6px;font-size:12px">Plantilla: <b>alion_co</b> | 1060826656745017</p>
<div class="upload"><b>📤 Subir Excel</b><br><small>Col A: Nombre | Col B: Numero con 57</small><br><br><input type="file" id="fileExcel" accept=".xlsx,.xls,.csv"><div id="infoExcel" style="margin-top:8px;color:#0B57D0;font-weight:bold"></div></div>
<textarea id="cts" rows="5" placeholder="Fer, 573001234567"></textarea>
...
<div id="res" style="text-align:center;margin-top:10px;font-weight:bold;white-space:pre-wrap"></div>
<h4>Log real de Meta (para ver por que no llega)</h4>
<pre id="log" style="background:#000;color:#0f0;padding:10px;font-size:11px;border-radius:8px;max-height:200px;overflow:auto"></pre>
<h4>Seguimiento</h4><table id="tb"><tr><th>Fecha</th><th>Enviados</th><th>Estado</th></tr></table>
</div>
<script>
document.getElementById('fileExcel').addEventListener('change', function(e){
 ...
 reader.onload=function(evt){
  ...
  for(let i=0;i<rows.length;i++){
   ...
   if(i==0 && (String(r[0]).toLowerCase().includes('nom') || String(r[1]).toLowerCase().includes('num'))) continue;
   let nombre=r[0]||'';
   let raw=String(r[1]||r[0]||'').replace(/[^0-9]/g,'');
   if(raw.length==10) raw='57'+raw;
   if(raw.length>=12) contacts.push(nombre+', '+raw);
  }
  document.getElementById('cts').value=contacts.join('\\n');
  document.getElementById('infoExcel').innerText='✅ '+contacts.length+' contactos listos (con 57)';
 };
 reader.readAsBinaryString(file);
});
async function send(){
 let raw=document.getElementById('cts').value.trim().split('\\n');
 let contacts=[]; for(let i=0;i<raw.length;i++){let tel=(raw[i].split(',')[1]||raw[i]).replace(/[^0-9]/g,''); if(tel.length==10) tel='57'+tel; if(tel.length>=12) contacts.push({tel:tel});}
 if(contacts.length==0){alert('Pon numero con 57');return;}
 document.getElementById('btn').innerText='Enviando...';
 document.getElementById('res').innerText=''; document.getElementById('log').innerText='';
 ...
 document.getElementById('log').innerText=JSON.stringify(d.details, null, 2);
 document.getElementById('btn').innerText='🚀 ENVIAR CAMPANA OFICIAL';
 load();
}
async function load(){let r=await fetch('/api/campaigns'); let data=await r.json(); let h='<tr><th>Fecha</th><th>Enviados</th><th>Estado</th></tr>'; for(let i=data.length-1;i>=0;i--){let c=data[i]; h+='<tr><td>'+c.fecha+'</td><td>'+c.sent+'</td><td>'+c.estado+'</td></tr>';} document.getElementById('tb').innerHTML=h;}
load();
</script></body></html>
`);
});
...
app.post('/api/send-campaign-template', async (req,res)=>{
 let sent=0; let errors=[]; let fecha=new Date().toLocaleString('es-CO');
 for(let c of req.body.contacts){
  try{
   let r=await fetch('https://graph.facebook.com/v20.0/'+PHONE_ID+'/messages',{method:'POST',headers:{'Content-Type':'application/json','Authorization':'Bearer '+TOKEN},body:JSON.stringify({messaging_product:'whatsapp',to:c.tel,type:'template',template:{name:'alion_co',language:{code:'es_CO'}}})});
   let j=await r.json();
   if(j.messages || j.contacts){sent++;} else {errors.push({tel:c.tel, error:j});}
   await new Promise(r=>setTimeout(r,800));
  }catch(e){errors.push({tel:c.tel, error:e.message});}
 }
 let msg = errors.length>0? '❌ ERROR META: '+(errors[0].error.error?.error_data?.details || errors[0].error.error?.message || JSON.stringify(errors[0].error)) + ' | Enviados: '+sent : '✅ Enviados: '+sent+' | Alcance: '+sent;
 campaigns.push({fecha:fecha,sent:sent,estado: errors.length>0? 'Error: '+JSON.stringify(errors[0].error).substring(0,60) : 'Enviada legal'});
 res.json({sent:sent, msg:msg, details: errors.length>0?errors:{ok:'todo enviado a Meta'}});
});
