...
// SYNC PLANTILLAS APROBADAS API
async function syncApprovedTemplates(){
  try{
    if(!process.env.WHATSAPP_BUSINESS_ID ||!process.env.WHATSAPP_TOKEN) return;
    const r = await fetch(`https://graph.facebook.com/v21.0/${process.env.WHATSAPP_BUSINESS_ID}/message_templates`,{
    ...
    console.log('Templates synced:', approved.length);
  }catch(e){ console.error('sync error',e.message); }
}
syncApprovedTemplates();
setInterval(syncApprovedTemplates, 1000*60*60);
