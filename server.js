...
async function initDB(){
 if(!process.env.DATABASE_URL){ console.log('⚠️ Sin DATABASE_URL - historial se perderá al reiniciar'); return; }
 try{
  ...
  console.log(`✅ DB OK - ${memMessages.length} msgs, ${Object.keys(unreadMap).length} sin leer`);
...
async function fetchMetaTemplates(){
 try{
  ...
  if(j.data){cacheTemplates=j.data.filter(t=>t.status==='APPROVED').map(t=>({name:t.name,language:t.language,category:t.category,components:t.components})); console.log(`✅ PLANTILLAS (${cacheTemplates.length}): ${cacheTemplates.map(t=>t.name).join(', ')}`);}
...
function saveHistory(m){
 ...
 if(pool){
  ...
  pool.query(`INSERT INTO messages(wa_id,text,type,direction,source,template_name,status,is_read) VALUES($1,$2,$3,$4,$5,$6,$7,$8)`,[m.wa_id,m.text,m.type,m.direction,m.source,m.template_name||null,m.status||'sent', m.is_read||false]).catch(()=>{});
 }
}

// --- LOGIN QUE TE FALTABA ---
app.post('/api/login',(req,res)=>{
 const email=(req.body.email||'').toLowerCase().trim();
 const pass=(req.body.password||'').trim();
 if(email===ADMIN_EMAIL && pass===ADMIN_PASS) return res.json({ok:true, token:'klido_ok'});
 res.json({ok:false, error:'Correo o contraseña incorrectos'});
});
...
app.post('/webhook',async(req,res)=>{
 ...
 if(v.statuses){
  ...
  if(memCampaigns.length){
   const c=memCampaigns.find(x=>x.status==='enviando')||memCampaigns[0];
   if(c){ if(st.status==='sent')c.sent++; if(st.status==='delivered')c.delivered++; if(st.status==='read')c.read++; if(st.status==='failed')c.failed++; if(pool) pool.query(`UPDATE campaigns SET sent=$1, delivered=$2, read=$3, failed=$4 WHERE id=$5`,[c.sent,c.delivered,c.read,c.failed,c.id]).catch(()=>{}); }
  }
 }
 if(v.messages){
  const m=v.messages[0];
  const text=m.type==='text'?m.text.body: m.type==='image'?'[Imagen]':`[${m.type}]`;
  saveHistory({wa_id:m.from,text,type:m.type,direction:'in',source:'inbox',created_at:new Date(),status:'delivered',is_read:false});
 }
 res.sendStatus(200);
});
...
app.get('/api/chats',async(_,res)=>{
 try{
  if(pool){
   const {rows}=await pool.query(`SELECT last_msg.wa_id, last_msg.text, last_msg.created_at, last_msg.source, last_msg.direction, last_msg.template_name, c.name, (SELECT COUNT(*) FROM messages WHERE wa_id=last_msg.wa_id AND direction='in' AND is_read=false) > 0 as unanswered, (SELECT COUNT(*) FROM messages WHERE wa_id=last_msg.wa_id AND direction='in' AND is_read=false) as unread_count FROM (SELECT DISTINCT ON (wa_id) wa_id, text, created_at, source, direction, template_name FROM messages ORDER BY wa_id, created_at DESC) last_msg LEFT JOIN contacts c ON c.wa_id=last_msg.wa_id ORDER BY last_msg.created_at DESC LIMIT 500`);
...
// MARCAR COMO LEIDO - ESTO TE FALTABA
...
// SUBIR ARCHIVO EXCEL / CSV - ESTO TE FALTABA
...
app.post('/api/campaigns/send-bulk',async(req,res)=>{
 ...
 const camp={id,name:templateName,total:numbers.length,sent:0,delivered:0,read:0,failed:0,status:'enviando',created_at:new Date().toISOString()}; memCampaigns.unshift(camp);
 if(pool) await pool.query(`INSERT INTO campaigns(id,name,total,sent,delivered,read,failed,status) VALUES($1,$2,$3,0,0,0,0,$4)`,[id,templateName,numbers.length,'enviando']).catch(()=>{});
 (async()=>{
  for(const raw of numbers){
   const to=String(raw).replace(/\D/g,'');
   try{
    const r=await fetch(`https://graph.facebook.com/v20.0/${PHONE_ID}/messages`,{method:'POST',headers:{Authorization:`Bearer ${META_TOKEN}`,'Content-Type':'application/json'},body:JSON.stringify({messaging_product:'whatsapp',to,type:'template',template:{name:templateName,language:{code:lang}}})});
    const j=await r.json();
    if(j.messages){camp.sent++; saveHistory({wa_id:to,text:`[Plantilla ${templateName}]`,type:'template',direction:'out',source:'campaign',template_name:templateName,status:'sent',created_at:new Date(),is_read:true});} else camp.failed++;
   }catch{camp.failed++;}
...
app.get('/health',async(_,res)=>res.json({ok:true,waba:WABA_ID||'auto',templates:cacheTemplates.map(t=>t.name),hasDB:!!pool,unread:Object.keys(unreadMap).length}));
app.get('/',(req,res)=>res.sendFile(path.join(publicPath,'login.html')));
app.listen(process.env.PORT||3000,()=>console.log(`🚀 KLIDO AZUL LISTO - LOGIN: ${ADMIN_EMAIL}`));
