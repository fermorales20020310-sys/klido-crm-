...
// FIX 2: LOGIN - para tu acceso con correo y clave
app.post('/api/login',(req,res)=>{
  ...
  res.json({ok:false,msg:'Correo o clave incorrectos'});
});
...
app.post('/api/campaigns/send-bulk',async(req,res)=>{
  const {numbers,message}=req.body; let sent=0; for(const raw of (numbers||[])){ const clean=String(raw).replace(/\D/g,''); if(clean.length<10) continue;
...
// FIX 3: RUTAS EXPLICITAS PARA QUE CAMPAÑA SIEMPRE ABRA + LOGO
...
// FIX 4: / debe ir a login si no hay auth, si no a index
app.get('/',(_,res)=>res.sendFile(path.join(__dirname,'public','login.html')));

app.listen(process.env.PORT||3000,()=>console.log('KLIDO CRM PRO OK - CAMPAÑA FIXED'));
