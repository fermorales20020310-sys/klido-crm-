...
async function init(){
await pool.query(`CREATE TABLE IF NOT EXISTS agencies(id TEXT PRIMARY KEY,name TEXT,phone_number_id TEXT UNIQUE,waba_id TEXT,created_at BIGINT)`);
...
try{const au=process.env.ADMIN_USER;const aa=process.env.ADMIN_AGENCY||process.env.DEFAULT_AGENCY||'acol';let h=process.env.ADMIN_PASSWORD_HASH;const pl=process.env.ADMIN_PASSWORD||process.env.ADMIN_PASS;if(au&&!h&&pl)h=bcrypt.hashSync(pl,8);if(au&&h)await pool.query(`INSERT INTO users(agency_id,username,password_hash,role) VALUES($1,$2,$3,'jefe') ON CONFLICT(agency_id,username) DO UPDATE SET password_hash=$3`,[aa,au,h]);}catch(e){}
console.log('GOLD MULTI-10 TODO-EN-UNO ON');
}
init();
app.use(express.static(path.join(__dirname,'public')));

// PANEL SUPER ADMIN integrado en /
app.get('/super',(req,res)=>{res.send(`<!DOCTYPE html><html><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>Super Admin Klido</title><style>body{font-family:system-ui;background:#0a1931;color:#fff;padding:20px}input{width:100%;padding:10px;margin:6px 0;border-radius:8px;border:1px solid #4da6ff;background:#071224;color:#fff}button{background:#1e90ff;color:#fff;border:0;padding:11px;border-radius:8px;width:100%;font-weight:700;cursor:pointer;margin-top:6px}.card{background:#0f2347;border:1px solid #4da6ff;border-radius:12px;padding:14px;margin:10px 0}</style></head><body><h2>KLIDO Super Admin (max 10 agencias)</h2><input id="k" placeholder="SUPER_ADMIN_KEY" type="password"><div class="card"><h3>Crear agencia</h3><input id="aid" placeholder="id agencia ej: agencia2"><input id="pid" placeholder="phone_number_id de Meta"><input id="waba" placeholder="waba_id (opcional)"><button onclick="cAg()">Crear agencia</button></div><div class="card"><h3>Crear usuario</h3><input id="uag" placeholder="agencia id"><input id="uus" placeholder="email usuario"><input id="upw" placeholder="contraseña"><button onclick="cUs()">Crear usuario</button></div><div class="card"><h3>Agencias</h3><button onclick="lAg()">Listar</button><div id="lst"></div></div><script>
async function cAg(){let r=await fetch('/api/super/agencies',{method:'POST',headers:{'Content-Type':'application/json','x-super-key':k.value},body:JSON.stringify({id:aid.value,phone_number_id:pid.value,waba_id:waba.value})});alert(await r.text());}
async function cUs(){let r=await fetch('/api/super/users',{method:'POST',headers:{'Content-Type':'application/json','x-super-key':k.value},body:JSON.stringify({agency_id:uag.value,username:uus.value,password:upw.value,role:'jefe'})});alert(await r.text());}
async function lAg(){let r=await fetch('/api/super/agencies',{headers:{'x-super-key':k.value}});let j=await r.json();lst.innerHTML='<pre>'+JSON.stringify(j,null,2)+'</pre>';}
</script></body></html>`);});
...
res.json({ok:true,msg:'Ahora agrega a Railway AGENCY_MAP: "'+phone_number_id+'":"'+nid+'" y redeploy'});
});
...
app.get('/api/messages/:wa',async(req,res)=>{const r=await pool.query(`SELECT text,direction,timestamp,media_type,media_url,is_campaign FROM messages WHERE wa_id=$1 AND agency_id=$2 ORDER BY timestamp ASC`,[req.params.wa,req.query.agency_id]);res.json(r.rows);});
